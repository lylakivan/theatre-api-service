...
class Performance(models.Model):
    play = models.ForeignKey(Play, on_delete=models.CASCADE, related_name='performance_play')
    ...
    def clean(self) -> None:
        if not (1 <= self.row <= self.performance.theatre_hall.rows):
            raise ValidationError({
                "row": [f"row number must be in available range: (1, {self.performance.theatre_hall.rows}):"]
            })
        if not (1 <= self.seat <= self.performance.theatre_hall.seats_in_row):
            raise ValidationError({
                "seat": [f"seat number must be in available range:(1, {self.performance.theatre_hall.seats_in_row})"]
            })
